 ...
 * 
 * @author Vincent Albanese, Zach Weinert, Cam Rosenthal
 * @version 2018.04.24
 */
public class Quad {
     ...
     * @param description
     *            the description of the point O(logn)
     */
    ...
    public ArrayList<Node<Point>> streetSearch(String streetname) {
        ...
        ArrayList<Node<Point>> list = new ArrayList<Node<Point>>();
        streetSearch(list, streetname);
       return list;
    }
    public ArrayList<Node<Point>> streetSearch(ArrayList<Node<Point>> list, String type ) {
        StreetNode a = new StreetNode(type);
        if (this.tree.find(a) != null) {
            list.addAll(a.getPlace());
           tree.remove(a);
    ...
    public ArrayList<Node<Point>> streetSearch(String streetName,
        ...
        if (tree.find(a) != null) {
            int index = tree.find(a).getPlace().size();
            for (int i = 0; i < index; i++) {
                if (tree.find(a).getPlace().get(i).getPlace()
                        .contains(type_of_place)) {
                    list.add(this.tree.find(a).getPlace().get(i));
                }
            }
        }
        return list;
    ...
    public ArrayList<Node<Point>> streetSearch(int originX, int originY,
        ...
        if (!inTheBound(p)) {
            System.out.println("Origin point is not in bounds");
            return null;
        }
    ...
    public void insert(int x, int y, String description, String... streets) {
        ...
        System.out.print(streets.length);
        for (int i = 0; i < streets.length; i++) {
            System.out.println("stop after add streest 1");
            ArrayList listt = new ArrayList<String>();
            listt.add(streets[i]);
            newNode.setStreets(listt);
            
            
            System.out.println("stop after add streest");
            StreetNode sNode = new StreetNode(streets[i]);
            System.out.print("stops after making new StreetNide");
            if (tree.find(sNode) == null) {
                sNode.addPoint(newNode);
                tree.insert(sNode);
                System.out.println("this stops in the if statement");
            }
            else {
               tree.find(sNode).getPlace().add(newNode);
               System.out.println("steops in the else");
            }
        }
        System.out.println("steops after loop");
